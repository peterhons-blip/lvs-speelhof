...
<p>
    ...
    School: {{ $school->schoolnaam ?? '—' }}<br><br>
</p>
...
<p>
    @foreach($payload as $l)
        ...
        Klas: {{ $l['klas'] ?? '—' }}<br>
        Geboortedatum: {{ optional($l['geboortedatum'])->format('d-m-Y') }}<br>
        ...
        Bericht langskomen secretariaat verstuurd: {{ !empty($l['secretariaat_bericht_verzonden']) ? 'JA' : 'NEE' }}<br><br>
    @endforeach
</p>
